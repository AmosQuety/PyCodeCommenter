parameter_descriptions = {
    "calculate_area": {
        "length": "Length of the rectangle.",
        "width": "Width of the rectangle.",
    },
    "calculate_volume": {
        "radius": "Radius of the sphere.",
        "height": "Height of the cylinder.",
    },
    "calculate_perimeter": {
        "side": "Side length of the square.",
        "length": "Length of the rectangle.",
        "width": "Width of the rectangle.",
    },
    "calculate_surface_area": {
        "radius": "Radius of the sphere.",
        "height": "Height of the cylinder.",
        "length": "Length of the cuboid.",
        "width": "Width of the cuboid.",
        "depth": "Depth of the cuboid.",
    },
    "calculate_density": {
        "mass": "Mass of the object.",
        "volume": "Volume of the object.",
    },
    "sort_list": {
        "lst": "List to be sorted.",
        "reverse": "Sort in descending order if True.",
    },
    "find_max": {"numbers": "List of numbers to find the maximum from."},
    "find_min": {"numbers": "List of numbers to find the minimum from."},
    "search_item": {
        "lst": "List to search the item in.",
        "item": "Item to search for in the list.",
    },
    "concatenate_strings": {"str1": "First string.", "str2": "Second string."},
    "split_string": {
        "string": "String to be split.",
        "delimiter": "Delimiter to split the string by.",
    },
    "read_file": {"file_path": "Path to the file to be read."},
    "write_file": {
        "file_path": "Path to the file to be written.",
        "content": "Content to write to the file.",
    },
    "connect_to_database": {
        "host": "Database host address.",
        "port": "Port number for the database connection.",
        "username": "Username for the database connection.",
        "password": "Password for the database connection.",
    },
    "execute_query": {
        "query": "SQL query to be executed.",
        "connection": "Database connection object.",
    },
    "send_request": {
        "url": "URL to send the request to.",
        "method": "HTTP method (GET, POST, etc.).",
        "headers": "Headers to include in the request.",
        "data": "Data to send in the request body.",
    },
    "send_email": {
        "to_address": "Recipient email address.",
        "subject": "Subject of the email.",
        "body": "Body content of the email.",
    },
}
